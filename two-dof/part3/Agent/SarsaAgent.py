...
class SarsaAgent(BaseAgent):
    ...
    __PARAMATER_FILE = "sarsa_param.json"       # パラメータを保存する JSON ファイル
    ...
    def reset(self) -> None:
        ...
        初期化 (エピソード開始時に呼ぶ)
        """
        # SARSAはステップごとにQ値を更新するため，メモリ不要
        pass


    def add(self, state: tuple, action: int, reward: float, next_state: tuple, next_action: int) -> None:
        """
        ...
        パラメータ
            ...
            next_action(int)   : 次の行動 (ε-greedy で選択済み)
        """
        key      = (state,      action)
        key_next = (next_state, next_action)

        # TD誤差の計算: r + γ * Q(s', a') - Q(s, a)
        td_error = reward + self._GAMMA * self.__Q[key_next] - self.__Q[key]
