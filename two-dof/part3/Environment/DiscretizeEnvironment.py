...
class _DiscretizeBaseEnvironment(ABC):
    ...
    def __init__(self, robot: PyBulletRobotController, state_goal: np.ndarray, sim_sleep_time_step: float) -> None:
        """
        コンストラクタ
    ...
    def __chk_override_constant(self) -> None:
        """
        定数のオーバーライド確認
        """
...
class TwoDOFEnv(_DiscretizeBaseEnvironment):
    ...
    def __cnvrt_state_to_joints(self, state: tuple) -> np.ndarray:
        """
        状態(インデックス)から関節角度に変換
    ...
    def __set_thetas_from_state(self, state: tuple) -> None:
        """
    ...
    def __next_state(self, action: int) -> tuple:
        """
        次状態の取得
    ...
    def __get_delta_thetas(self, action: int) -> np.ndarray:
        """
        関節角度の差分を計算
    ...
    def __discretize_state(self, state: np.ndarray) -> tuple:
        """
        状態の離散化
    ...
    def __calc_reward(self, dist: float) -> float:
        """
        報酬の取得
    ...
    def __is_closed(self, dist: float) -> bool:
        """
