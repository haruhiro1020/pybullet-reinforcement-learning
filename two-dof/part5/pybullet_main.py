...
class MainPyBulletRobot:
    ...
    メソッド
        ...
        private
            ...
            メイン処理関連
                __learning(): 強化学習の学習フェーズ (DQN / Q学習)
    ...
    __MAX_INFERENCE_STEPS =  7000   # 推論フェーズの最大ステップ数 (0.001s/step × 7000 ≈ 7s, 10s制限の目安)
    ...
    def __learning(self):
        ...
        DQNはoff-policy TD学習のため，1ステップごとに経験をバッファに追加してネットワークを更新する．
        Q学習との差分は done フラグを add() に渡す点のみである．
        """
        # 全エピソードの報酬
        rewards_history = []
        ...
        for episode in range(self.__MAX_EPISODES):
            ...
            while not done:
                ...
                # 経験バッファに追加 + ミニバッチ学習
                # done フラグを渡す点が Q学習との唯一の違い
                self.__agent.add(state, action, reward, next_state, done=done)
    ...
    def __inference(self):
        ...
        while not done:
            ...
            # 最大ステップ数を超えたら推論失敗として打ち切る
            if count >= self.__MAX_INFERENCE_STEPS:
                print(f"[WARNING] 推論フェーズが最大ステップ数 ({self.__MAX_INFERENCE_STEPS}) に達しました。目標未達。")
                break
